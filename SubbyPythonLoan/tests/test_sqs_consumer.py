...
The state matrix asserted here is the contract Java's at-least-once SQS delivery
relies on (see `_claim` in src/messaging/sqs_consumer.py):

  NEW           — first delivery, proceed to handle
  RETRY         — prior FAILED row with attempts < MAX_RETRIES, reclaim
  SKIP_OK       — row already SUCCEEDED (or FAILED+exhausted), ACK
  SKIP_INFLIGHT — row PENDING (another replica owns it), leave for redelivery
"""
...
async def test_claim_returns_NEW_first_time(consumer, session_factory):
    ...
    state = await consumer._claim(str(uuid.uuid4()))
